...
using Triton.FleetManagement.WebApi.Helper;
    ...
    public class BookingsRepository : IBookings
    {
        ...
        public async Task<BookingsModel> GetByIdAsync(int bookingsID)
        {
            ...
            const string sql = "EXEC TritonFleetManagement.dbo.proc_Bookings_GetBookingsByID @BookingsID " +
                               ...
                               "EXEC [TritonGroup].[dbo].[proc_LookupCodes_By_CategoryID_Select] 102 " +
                               "EXEC [TritonFleetManagement].[dbo].[proc_PartReason_GetByBookingID] @BookingsID " +
                               "EXEC [TritonFleetManagement].[dbo].[proc_OutworksCommission_GetBookingsByID] @BookingsID " +
                               "EXEC [TritonGroup].[dbo].[proc_LookUpCodes_ByCategoryID_Select] 102 "
                               ;
            ...
            using (var multi = connection.QueryMultiple(sql, new { BookingsID = bookingsID }))
            {
                ...
                bookingsModel.Parts = multi.Read<LookupCodeModel>().ToList();
                bookingsModel.PartReasons = multi.Read<PartReason>().ToList();
                bookingsModel.OutworksCommission = multi.Read<OutworksCommission>().FirstOrDefault();
                bookingsModel.LookupCodesPerCategory = multi.Read<LookupCodeModel>().ToList();
            }
        ...
        public async Task<bool> InsertAsync(BookingsModel bookingsModel)
        {
            try
            {
                ...
                new
                {
                    ...
                    bookingsModel.Bookings.OutworksCommission,
        ...
        public async Task<BookingsModel> LookUpCodesAsync(int customerID)
        {
            ...
            using (var multi = connection.QueryMultiple(query, new { CustomerID = customerID }))
            {
                ...
                bookingModel.Parts = multi.Read<LookupCodeModel>().ToList();
        ...
        public async Task<proc_Bookings_BookingReasons_Customers_Select> GetBookingReasonsPerJobCard(int bookingsID)
        {
            const string sql = "[TritonFleetManagement].[dbo].[proc_Bookings_BookingReasonsJobCard_Customers_Select]";
            await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
            return connection.Query<proc_Bookings_BookingReasons_Customers_Select>(sql, new { bookingsID }, commandType: CommandType.StoredProcedure).FirstOrDefault();
        }
        ...
        public async Task<proc_BookingDetails_GetByID> GetBookingDetailsByCustomerID(int CustomerID)
        {
            const string sql = "[TritonFleetManagement].[dbo].[proc_BookingDetails_GetByCustomerID]";
            await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
            return connection.Query<proc_BookingDetails_GetByID>(sql, new { CustomerID }, commandType: CommandType.StoredProcedure).FirstOrDefault();
        }
        ...
        public async Task<bool> UpdatePartReasons(BookingsModel model)
        {
            try
            {
                var PartReason = Triton.Core.Extensions.ToDataTableFromList(model.PartsList, true);
                var LabourReason = Triton.Core.Extensions.ToDataTableFromList(model.LabourList, true);
                var ConsumablesReason = Triton.Core.Extensions.ToDataTableFromList(model.ConsumablesList, true);
                var TyreReason = Triton.Core.Extensions.ToDataTableFromList(model.TyresList, true);
                await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
                var data = connection.Query<bool>("proc_PartReason_Update", new
                {
                    PartReason = PartReason.AsTableValuedParameter("Part_Reason_tvp"),
                    LabourReason = LabourReason.AsTableValuedParameter("Labour_Reason_tvp"),
                    ConsumablesReason = ConsumablesReason.AsTableValuedParameter("Consumable_Reason_tvp"),
                    TyreReason = TyreReason.AsTableValuedParameter("Tyre_Reason_tvp")
        ...
        public async Task<List<PartReason>> GetPartsPerBooking(int BookingsID)
        {
            string sql = @"SELECT 
                                PR.BookingsID,
                                PR.InventoryID,
	                            PR.VehicleID
                            FROM TritonFleetManagement.dbo.PartReason PR 
                            INNER JOIN TritonFleetManagement.dbo.Inventory I ON I.InventoryID = PR.InventoryID
                            WHERE PR.BookingsID = @BookingsID";


            await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));

            return connection.Query<PartReason>(sql, new { BookingsID = BookingsID }).ToList();
        }

        public async Task<bool> InsertPartsBookingReasonAndOutworksCommissionAsync(BookingsModel bookingsModel)
        {
            try
            {
                await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
                return connection.Query<bool>("[TritonFleetManagement].[dbo].[proc_BookingsOutworksCommissionPartsAndBookingReason_Insert]",
                new
                {
                    bookingsModel.OutworksCommission.BookingID,
                    bookingsModel.OutworksCommission.VehicleID,
                    bookingsModel.OutworksCommission.IsOutworksCommission,
                    bookingsModel.OutworksCommission.CreatedBy,
                    bookingsModel.OutworksCommission.SupplierDescription,
                    bookingsModel.OutworksCommission.PONumber,
                    bookingsModel.OutworksCommission.Quantity,
                    bookingsModel.OutworksCommission.Isssued,
                    bookingsModel.OutworksCommission.CostExVAT,
                    bookingsModel.OutworksCommission.SellingPrice
        ...
        public async Task<bool> UpdatePartsBookingReasonAndOutworksCommissionAsync(BookingsModel bookingsModel)
        {
            try
            {
                await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
                DBConnection.GetContextInformationFromConnection(connection, bookingsModel.Bookings.CreatedByUserID);
                bookingsModel.Bookings.CreatedOn = DateTime.Now;
                return connection.Query<bool>("[TritonFleetManagement].[dbo].[proc_BookingsOutworksCommissionPartsAndBookingReason_Update]",
                new
                {
                    bookingsModel.Bookings.BookingsID,
                    bookingsModel.Bookings.CustomerID,
        ...
        public async Task<PartReasonViewModel> GetInventoryByBookingID(int BookingsID)
        {
            await using var connection = DBConnection.GetOpenConnection(_config.GetConnectionString(StringHelpers.Database.TritonFleetManagement));
            var partModel = new PartReasonViewModel();

            var query = string.Format(@" 
                                        EXEC [TritonFleetManagement].[dbo].[proc_InventoryByBookingsID_Select] @BookingsID
                                        EXEC [TritonFleetManagement].[dbo].[proc_OutworksCommission_GetBookingsByID] @BookingsID
                                        "
                                     );
            using (var multi = connection.QueryMultiple(query, new { BookingsID = BookingsID }))
            {
                partModel.InventoryCustoModelList = multi.Read<InventoryCustoModel>().ToList();
                partModel.OutworksCommission = multi.Read<OutworksCommission>().FirstOrDefault();
            }

            var BookingsHelper = new BookingsHelper();
            partModel = BookingsHelper.GetPartsByCategory(partModel);
            return partModel;
        }        
    }
}
