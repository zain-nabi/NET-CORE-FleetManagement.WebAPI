...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[proc_ScaleHour_ScaleHoursByCostScale_Select]')
AND type in (N'P', N'PC'))

PRINT 'YES'
GO 

CREATE PROC proc_ScaleHour_ScaleHoursByCostScale_Select
(
	@costScale			INT
)
AS 
BEGIN
	...
	WHERE SC.CostScaleHour = @costScale
END