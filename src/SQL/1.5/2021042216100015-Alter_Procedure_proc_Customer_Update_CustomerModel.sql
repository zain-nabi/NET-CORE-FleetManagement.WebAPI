...
ALTER PROCEDURE  proc_Customer_Update_CustomerModel
(
	/*Customer Parameters*/
	@CustomerID               INT,
    @Name                     VARCHAR(255),
...
BEGIN TRY
   ...
   UPDATE Customer 
	SET 
	    Name = @Name,  AccountStatusTypeLCID = @AccountStatusTypeLCID,CompanyID = @CompanyID,
		...
		VatRegistration = @VatRegistration, PostalSuburb = @PostalSuburb, CompanyRegistration = @CompanyRegistration
  ...
  DECLARE @CompanyIDTTM INT
      ...
      SELECT @CompanyIDTFM = C.CompanyID FROM [TritonSecurity].[dbo].[Companys] C WITH(NOLOCK)    WHERE C.Description ='Triton Fleet Management Services'    
      ...
      INSERT INTO [dbo].[VendorCode]([CustomerID],[CompanyID],[VendorCodes])
      VALUES

      (@CustomerID,@CompanyIDTFM,@VendorCodeTFM ),
      (@CustomerID,@CompanyIDTTM,@VendorCodeTFM )
  END
